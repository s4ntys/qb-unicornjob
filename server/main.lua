            ...
            if billed ~= nil then
                if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                    if amount and amount > 0 then
                        ...
                        TriggerClientEvent('QBCore:Notify', source, 'Musí to byť platná suma vyššia ako 0', 'error')
                    end
                else
                    TriggerClientEvent('QBCore:Notify', source, 'Nemôžete si vystaviť účet', 'error')
                end
            else
                TriggerClientEvent('QBCore:Notify', source, 'Hráč nie je online', 'error')
            end
        else
